...
class Gemini38Preview(unittest.TestCase):
    def test_synthesizes_with_interactions_and_decodes_wav(self):
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}), patch.object(
            ...
            result = voice_preview._synth_gemini_api("Achernar", "gemini-3.8-flash-lite-tts", "नमस्ते")
        ...
        self.assertEqual(payload["input"][0]["content"][0]["text"], "नमस्ते <laugh>")
        self.assertEqual(payload["input"][0]["content"][0]["annotations"][0]["type"], "speech_metadata")
    ...
    def test_missing_gemini_key_is_clear(self):
        with patch.dict(os.environ, {}, clear=True):
            with self.assertRaisesRegex(voice_preview.PreviewError, "no Gemini API key"):
                voice_preview._synth_gemini_api("Achernar", "gemini-3.8-flash-lite-tts", "Hello")
    ...
    def test_flash_prefix_uses_flash_model_on_interactions_route(self):
        audio = b"RIFF-test-wave"
        ...
        with patch.dict(os.environ, {"GEMINI_API_KEY": "test-key"}), patch.object(
            ...
            voice_preview.synthesize("google38flash:achernar", "en")
        self.assertEqual(post.call_args.args[2]["model"], "gemini-3.8-flash-tts")
