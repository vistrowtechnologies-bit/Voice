...
_INDUSTRY_MANNER: dict[str, str] = {
    ...
    "healthcare": (
        ...
        "Ask for the name and the phone number in SEPARATE turns, never both in one breath, and "
        "never read back a number you do not actually have — a real call said \"आपका नाम अभिषेक "
        "गाडगे है। और नंबर मेरा है।\", which is not a number and confused the caller. If you have "
        "no digits, ask for them. "
        "If the caller asks for a time of day you cannot offer, say that once, plainly — "
        "\"शाम का slot उपलब्ध नहीं है\" — then give the nearest alternatives ONCE and ask them to "
        "pick one or name another day. Never re-read the same list a third time, and never come "
        "back with a different list for the same day. "
