"""Guard both Gemini 3.8 model families and the LiveKit direct-API route."""
import os
...
class Google38Voices(unittest.TestCase):
    ...
    def test_testing_only_premium_with_credits_in_name(self):
        for v in self.entries():
            ...
            self.assertIn("Flash-Lite", v["name"])
            self.assertIn(v["gender"], ("male", "female"))
        for v in self.flash_entries():
            self.assertTrue(v["preview"], v["value"])
            self.assertEqual(v["tier"], "premium", v["value"])
            self.assertIn("Flash", v["name"])
    ...
    def test_model_mapping(self):
        ...
        self.assertEqual(
            voice_catalog.gemini_prefix_and_model("google38flash:kore"),
            ("google38flash:", "gemini-3.8-flash-tts"),
        )
